...
bool Queen::MakeMove(const ChessSquare& Move2)
{	
...
bool Queen::CheckForCapture(const ChessSquare& MoveTo)
{
	//ChessAssert();
	/*if (bIsWhite)
	{
...
bool Queen::IsAttackingSquare(const ChessSquare& Square) const
{
	...
	/*if ( isBlockingCheck() )
	{
		...
		else if (diffx == 0)
		{
			for (int i = 1; i < abs(diffy); i++)
			{
				...
				{
					PotMoves.push_back(ChessSquare( currLoc.row, 
												diffy > 0 ? currLoc.col + i : currLoc.col - i ) );
				}
